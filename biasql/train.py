...
class Agent:
    ...
    def learn(self, junction):
        ...
        # Bias Q-learning update
        max_next_action = torch.argmax(q_next, dim=1).unsqueeze(1)
        q_next_max = q_next.gather(1, max_next_action).squeeze(1)
        q_target = reward_batch + self.gamma * q_next_max
        q_target -= self.bias_weight * (q_eval - q_next_max).abs()  # Bias term
        ...
        if train:
            ...
            traci.start(
                [
                    ...
                    "tripinfo.xml",
                    "--emission-output",
                    "my_emission_file.xml",
                    "--fcd-output",
                    "sumoTrace.xml",
                    "--lanechange-output",
                    "my_lane_change_file.xml",
                    "--netstate-dump",
                    "my_dump_file.xml",
                ]
            )
        ...
        while step <= steps:
            ...
            for junction_number, junction in enumerate(all_junctions):
                ...
                if traffic_lights_time[junction] == 0:
                    ...
                    next_state = state_
                    next_state[action] += 1  # Assuming action is changing one lane's light
