...
def run(train=True,model_name="model",epochs=50,steps=500,ard=False):
    ...
    for e in range(epochs):
        if train:
            traci.start(
            [checkBinary("sumo"), "-c", "configuration.sumocfg", "--tripinfo-output", "tripinfo.xml", "--emission-output", "my_emission_file.xml", "--fcd-output", "sumoTrace.xml", "--lanechange-output", "my_lane_change_file.xml", "--netstate-dump", "my_dump_file.xml",]
            )
        else:
            traci.start(
            [checkBinary("sumo-gui"), "-c", "configuration.sumocfg", "--tripinfo-output", "tripinfo.xml", "--emission-output", "my_emission_file.xml", "--fcd-output", "sumoTrace.xml", "--lanechange-output", "my_lane_change_file.xml", "--netstate-dump", "my_dump_file.xml",]
            )
